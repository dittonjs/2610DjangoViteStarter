...
from .utils import LOGIN_PAGE, HOME_PAGE, CURRENT_USER, ASSET, ASSET_URL
...
def users_new(req: HttpRequest) -> HttpResponse:
    if req.user.is_authenticated:
        return redirect(f"/users/{req.user.id}/")
    if req.method == "GET":
        return render(req, "users/new.html", {ASSET: ASSET_URL})
...
def users_login(req: HttpRequest) -> HttpResponse:
    if req.user.is_authenticated:
        return redirect(f"/users/{req.user.id}/")
    if req.method == "GET":
        return render(req, "users/login.html", {ASSET: ASSET_URL})
...
def users_id(req: HttpRequest, user_id: int) -> HttpResponse:
    ...
    return render(req, "users/user.html", {ASSET: ASSET_URL, CURRENT_USER: req.user})
